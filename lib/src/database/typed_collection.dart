...
class TypedCollection<T> {
  ...
  Future<void> save(T item, String id) {
    return _repo.saveMap(collection: collectionName, id: id, data: toMap(item));
  }

  Future<List<T>> fetch({List<QueryFilter>? filters}) async {
    final list = await _repo.fetchMap(
      collection: collectionName,
      filters: filters,
    );
    return list
        .map((map) => fromMap(map, map[primaryKeyField]?.toString() ?? ''))
        .toList();
  }
  ...
  Stream<List<T>> watch({List<QueryFilter>? filters}) {
    return _repo
        ...
        .map(
          (list) => list
              .map(
                (map) => fromMap(map, map[primaryKeyField]?.toString() ?? ''),
              )
              .toList(),
        );
  }
